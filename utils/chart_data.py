...
flags.DEFINE_string('save_dir', './charts',
                    'Directory to save charts.')
flags.DEFINE_string('dataset', 'mnist', 'Dataset to analyze.')
flags.DEFINE_string(
    'sampling_methods',
    'uniform,margin',
  ...
  for k in sorted(all_results.keys()):
    ...
    if (isinstance(k, tuple)) and k[fields['score_method']] == score_method:
      ...
      try:
        ...
        results['accuracy'] = np.array(results['accuracy']).reshape((1,-1))
        results['data_sizes'] = np.array(results['data_sizes']).reshape((1,-1))
      ...
      if sampler == 'uniform':
        ...
        try:
          plt.fill_between(x, mean_acc - CI_acc,mean_acc + CI_acc,color=colors[sampler],alpha=0.2)
        except:
          print('erro fill_between')
  plt.legend(loc=4)
